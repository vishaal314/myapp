...
if docker exec dataguardian-container test -f /app/license.json; then
    test_pass "Database" "PostgreSQL connection available"
else
    test_warn "Database" "Check connection"
fi

# Test 1.5: No critical errors in logs
if docker logs dataguardian-container 2>&1 | tail -100 | grep -qi "critical\|fatal"; then
    test_fail "Application Logs" "Critical errors detected"
else
...
if docker logs dataguardian-container 2>&1 | grep -q "License loaded: DGP-ENT"; then
    ...
    test_fail "License Loading" "License not loaded properly"
fi

# Test 2.2: No license errors
if docker logs dataguardian-container 2>&1 | tail -100 | grep -qi "License Error"; then
    test_fail "License Validation" "License errors detected"
else
...
SCANNERS=(
    "Code Scanner:code:Python/JavaScript PII detection"
    "Website Scanner:website:URL compliance scanning"
    "Database Scanner:database:SQL PII detection"
    "Blob/File Scanner:blob:Document scanning"
    "Image Scanner:image:OCR-based PII detection"
    "AI Model Scanner:ai_model:Bias detection"
    "DPIA Scanner:dpia:Impact assessment"
    "SOC2 Scanner:soc2:Security compliance"
    "Sustainability Scanner:sustainability:Carbon footprint"
    "API Scanner:api:REST endpoint scanning"
    "Enterprise Connector:enterprise:Microsoft 365/Google"
    "Document Scanner:document:PDF/Word scanning"
)

for scanner in "${SCANNERS[@]}"; do
    IFS=':' read -r name type desc <<< "$scanner"
    test_info "$name" "$desc"
done
...
if docker exec dataguardian-container python3 -c "import reportlab" 2>/dev/null; then
    ...
    test_fail "PDF Report Library" "ReportLab not installed"
fi

# Test 4.2: Report directories exist
if docker exec dataguardian-container test -d /app/reports; then
    test_pass "Report Directory" "Exists"
else
    test_warn "Report Directory" "Not found"
fi

# Test 4.3: Report formats
...
if curl -s -I "$SERVER_URL" | grep -q "HTTP/2 200"; then
    test_pass "HTTPS" "Enabled with HTTP/2"
elif curl -s -I "$SERVER_URL" | grep -q "200 OK"; then
...
MEMORY=$(docker stats dataguardian-container --no-stream --format "{{.MemUsage}}" | awk '{print $1}')
