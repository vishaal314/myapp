...
class RiskMeter(Flowable):
    ...
    def draw(self):
        ...
        # Add risk level text
        d.add(String(self.width/2-20, 10, f"{self.risk_level} Risk", fontSize=12, fillColor=indicator_color))
...
class ComplianceBanner(Flowable):
    ...
    def draw(self):
        ...
        # Add subtitle based on compliance level
...
class SustainabilityMeter(Flowable):
    ...
    def __init__(self, sustainability_score):
        Flowable.__init__(self)
    ...
    def draw(self):
        ...
        description = String(10, bar_y - 30, 
                            "Measures data minimization, retention policies & processing efficiency", 
                            fontSize=8, 
                            fillColor=HexColor('#7f8c8d'))
        d.add(description)
...
def generate_report(scan_data: Dict[str, Any], 
    ...
    elements.append(Paragraph('GDPR Compliance Scan Report', subheading_style))
    
    # Current date with nice formatting
    ...
    elements.append(Paragraph(f"Scan ID: {display_scan_id}", scan_id_style))
    elements.append(Spacer(1, 24))
    
    # Executive Summary - styled as a highlight box
    ...
    # Create data for the summary table with nicer labels
    summary_data = [
        ['Scan Type', scan_type],
        ['Region', region],
        ['Date & Time', timestamp],
    ...
    # Add GDPR fine protection banner
    gdpr_banner = ComplianceBanner(compliance_level)
    elements.append(gdpr_banner)
    elements.append(Spacer(1, 12))
    
    # Add custom risk meter visual
    risk_meter = RiskMeter(risk_level)
    ...
    # Add sustainability compliance section
    elements.append(Paragraph('Data Sustainability Compliance', subheading_style))
    elements.append(Spacer(1, 6))
    
    # Add description of what sustainability means in this context
    sustainability_desc = """
    ...
    # Add the sustainability meter
    sustainability_meter = SustainabilityMeter(sustainability_score)
    ...
    if include_recommendations:
        ...
        elements.append(Paragraph('Data Sustainability Recommendations', subheading_style))
