...
from services.auth import is_authenticated, require_permission

# Check authentication - redirect to main app if not logged in
if not is_authenticated():
    st.warning("Please log in to access the SOC2 Compliance Scanner.")
    st.info("This feature is only available to authenticated users.")
    st.stop()

# Also check permission
if not require_permission('scan:create'):
    st.warning("You don't have permission to use the SOC2 Compliance Scanner.")
    st.info("This feature requires scan creation permissions.")
    st.stop()
