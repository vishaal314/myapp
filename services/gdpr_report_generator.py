...
class GDPRReportGenerator:
    ...
    def _add_title_section(self, content: List, scan_result: Dict[str, Any]):
        ...
        if not os.path.exists(logo_path):
            ...
            header_data = [[
                Paragraph(
                    "DataGuardian Pro",
                    ParagraphStyle(name='CompanyName',
        ...
        title_table.setStyle(
            TableStyle([
                ('BACKGROUND', (0, 0), (-1, -1),
                 colors.HexColor('#1e40af')),  # Dark blue background
                ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ...
        if not repo_url:
            repo_url = scan_result.get('source',
                                       {}).get('url', 'Unknown Repository')
        ...
        content.append(
            Paragraph(f"Full Path: {repo_url}", self.styles['Normal']))
        content.append(Spacer(1, 0.3 * inch))

        # Metadata with improved styling
        scan_timestamp = scan_result.get(
            'scan_timestamp',
        ...
        metadata = [["Scan Date:", scan_timestamp],
                    ["Scan ID:",
                     scan_result.get('scan_id', 'Unknown')],
                    ["Branch:",
                     scan_result.get('branch', 'Unknown')],
                    [
                        "Report Generated:",
        ...
        metadata_table.setStyle(
            TableStyle([
                ...
                ('BACKGROUND', (0, 0), (0, -1),
                 colors.HexColor('#f3f4f6')),  # Light gray background
    ...
    def _add_summary_section(self, content: List, scan_result: Dict[str, Any]):
        ...
        high_risk_text = Paragraph(
            f"• <font color='#dc2626'><b>{high_risk}</b></font> high-risk findings",
            ParagraphStyle(name='HighRiskItem',
                           parent=self.styles['EnhancedNormal'],
                           leftIndent=20))
        content.append(high_risk_text)

        medium_risk_text = Paragraph(
            f"• <font color='#ea580c'><b>{medium_risk}</b></font> medium-risk findings",
            ParagraphStyle(name='MediumRiskItem',
                           parent=self.styles['EnhancedNormal'],
                           leftIndent=20))
        content.append(medium_risk_text)

        low_risk_text = Paragraph(
            f"• <font color='#15803d'><b>{low_risk}</b></font> low-risk findings",
        ...
        score_data = [["GDPR Compliance Score", ""]]
        ...
        score_container_data = [["Overall GDPR Compliance Score"],
                                [score_circle], [f"out of 100"],
                                [score_label_table]]

        score_container = Table(score_container_data, colWidths=[2 * inch])
        score_container.setStyle(
            TableStyle([
                ('BACKGROUND', (0, 0), (-1, -1), colors.white),
                ('TEXTCOLOR', (0, 0), (0, 0),
                 colors.HexColor('#1e40af')),  # Blue title
                ('TEXTCOLOR', (0, 2), (0, 2),
                 colors.HexColor('#6b7280')),  # Gray "out of 100"
        ...
        stats_table.setStyle(
            TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0),
                 colors.HexColor('#f3f4f6')),  # Light gray header
                ('TEXTCOLOR', (0, 0), (-1, 0),
                 colors.HexColor('#4b5563')),  # Gray header text
    ...
    def _add_findings_section(self, content: List, scan_result: Dict[str,
                                                                     Any]):
        """Add the detailed findings section to the report."""
        content.append(
            Paragraph("Detailed Findings", self.styles['SectionTitle']))
            ...
            for finding in raw_findings:
                if isinstance(finding, dict) and 'type' in finding:
                    ...
                    findings.append({
                        ...
                        risk_level,
                        'gdpr_principle':
                        finding.get('gdpr_principle', 'data_minimization'),
                        'description':
        ...
        if not findings:
            content.append(
                Paragraph("No specific findings were identified.",
                          self.styles['Normal']))
            content.append(Spacer(1, 0.25 * inch))
            return
        ...
        if high_risk_findings:
            content.append(
                Paragraph("High Risk Findings", self.styles['HighRisk']))
            content.append(Spacer(1, 0.1 * inch))
            ...
            findings_table = Table(
                findings_data, colWidths=[1.2 * inch, 2.3 * inch, 3 * inch])
        ...
        if medium_risk_findings:
            content.append(
                Paragraph("Medium Risk Findings", self.styles['MediumRisk']))
            content.append(Spacer(1, 0.1 * inch))
        ...
        if low_risk_findings:
            content.append(
                Paragraph("Low Risk Findings", self.styles['LowRisk']))
            content.append(Spacer(1, 0.1 * inch))
    ...
    def _add_gdpr_principles_section(self, content: List,
                                     scan_result: Dict[str, Any]):
        """Add the GDPR principles section to the report."""
        content.append(
            Paragraph("GDPR Principles Analysis", self.styles['SectionTitle']))
        content.append(Spacer(1, 0.1 * inch))
        ...
        for key, principle in all_principles.items():
            ...
            principles_data.append([
                principle['title'], principle['article'], status,
                principle['description']
            ])

        principles_table = Table(
            principles_data,
            colWidths=[1.5 * inch, 0.8 * inch, 1 * inch, 3.2 * inch])
    ...
    def _add_recommendations_section(self, content: List,
                                     scan_result: Dict[str, Any]):
        """Add the recommendations section to the report."""
        content.append(
            Paragraph("Recommendations for Compliance",
        ...
        for recommendation in recommendations:
            content.append(
                Paragraph(f"• {recommendation}", self.styles['Normal']))
            content.append(Spacer(1, 0.1 * inch))

        content.append(Spacer(1, 0.25 * inch))

    def _add_netherlands_compliance_section(self, content: List,
                                            scan_result: Dict[str, Any]):
        """Add Netherlands-specific compliance section to the report."""
        import streamlit as st
        lang = st.session_state.get('language', 'en')

        section_title = "UAVG-specifieke naleving" if lang == 'nl' else "Dutch GDPR (UAVG) Specific Compliance"
        content.append(Paragraph(section_title, self.styles['SectionTitle']))
        content.append(Spacer(1, 0.1 * inch))

        # The rest of the logic for findings and issues remains unchanged...

        # Replace final static Netherlands table
        if lang == 'nl':
            nl_requirements = [
                ["Vereiste", "Beschrijving", "Rechtsgrond"],
                [
                    get_text("netherlands_regulatory.bsn_processing",
                             "BSN-verwerking"),
                    get_text(
                        "netherlands_regulatory.bsn_requirements",
                        "BSN mag alleen worden verwerkt wanneer uitdrukkelijk bij wet toegestaan"
                    ),
                    get_text("netherlands_regulatory.bsn_article",
                             "UAVG artikel 46")
                ],
                [
                    get_text("netherlands_regulatory.medical_data",
                             "Medische gegevens"),
                    get_text(
                        "netherlands_regulatory.medical_requirements",
                        "Medische gegevens vereisen uitdrukkelijke toestemming en aanvullende waarborgen"
                    ),
                    get_text("netherlands_regulatory.medical_article",
                             "UAVG artikel 30")
                ],
                [
                    get_text("netherlands_regulatory.minor_consent",
                             "Toestemming minderjarigen"),
                    get_text(
                        "netherlands_regulatory.minor_requirements",
                        "Ouderlijke toestemming vereist voor kinderen onder de 16 jaar"
                    ),
                    get_text("netherlands_regulatory.minor_article",
                             "UAVG artikel 5")
                ],
                [
                    get_text("netherlands_regulatory.data_breach_notification",
                             "Meldplicht datalekken"),
                    get_text(
                        "netherlands_regulatory.breach_requirements",
                        "De AP vereist melding binnen 72 uur voor significante datalekken"
                    ),
                    get_text("netherlands_regulatory.breach_article",
                             "AVG artikel 33")
                ]
            ]
        else:
            nl_requirements = [
                ...
                [
                    "BSN Processing",
                    "Dutch Citizen Service Number (BSN) may only be processed when explicitly authorized by law.",
                    "UAVG Article 46"
                ],
                [
                    "Medical Data",
                    "Medical data requires explicit consent and additional safeguards.",
                    "UAVG Article 30"
                ],
                [
                    "Minors Consent",
                    "Parental consent required for children under 16 years (higher than some EU countries).",
                    "UAVG Article 5"
                ],
                [
                    "Data Breach",
                    "The Dutch DPA (AP) requires notification within 72 hours for significant breaches.",
                    "GDPR Article 33"
                ]
            ]

        nl_table = Table(nl_requirements,
...
def generate_gdpr_report(
        scan_result: Dict[str, Any],
        output_dir: str = "reports") -> Tuple[bool, str, Optional[bytes]]:
    """
    ...
    try:
        ...
        success, report_path = report_generator.generate_pdf_report(
            scan_result)
