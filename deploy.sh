...
# Safe logging function - creates log directory if needed
log() {
...
# Create docker-compose.prod.yml with proper service definitions
create_docker_compose() {
    log "Creating docker-compose.prod.yml..."
...
services:
  ...
  postgres:
    ...
    networks:
      - dataguardian-network
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${DB_USER} -d dataguardian_pro"]
      interval: 30s
      timeout: 10s
      retries: 3
...
RUN mkdir -p data reports cache logs
...
# Run certbot renew using the built-in certbot container
if docker exec dataguardian-certbot certbot renew --quiet 2>/dev/null; then
...
check_services() {
    ...
    # Check using the actual container names from docker-compose.prod.yml
    for service in dataguardian-pro dataguardian-postgres dataguardian-redis dataguardian-nginx dataguardian-certbot; do
