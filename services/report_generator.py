...
def _add_sustainability_report_content(elements, scan_data, styles, heading_style, subheading_style, normal_style, current_lang, include_details=True, include_charts=True, include_recommendations=True):
    ...
    # Email ID of logged in user should be used for the username field
    # Only fall back to "DataGuardian Pro User" if no email is available
    username = None  # Will be populated with the email ID if found
        ...
        if hasattr(st, 'session_state'):
            ...
            # If username is not in session, try user_info
                ...
                # Fallback to other user identifiers if email is not available
                    ...
                    logger.info(f"Using username from user_info (email not available): {username}")
                elif 'name' in user_info and user_info['name'] and user_info['name'] != 'Anonymous':
                    username = user_info['name']
                    logger.info(f"Using name from user_info (email and username not available): {username}")
            ...
            # Also check current_user in session state
                ...
                if isinstance(st.session_state.current_user, dict):
                    ...
                    # Fall back to username if email is not available
                        ...
                        logger.info(f"Using username from current_user (email not available): {username}")
                # If current_user is just a string, use it directly
    ...
    if include_code_stats and include_details:
        ...
        code_stats_data = [
            [_("report.total_files", "Total Files"), str(code_stats.get('total_files', file_count))],
            [_("report.repository_size", "Repository Size"), f"{repo_size_mb:.2f} MB"],
            [_("report.sustainability_score", "Sustainability Score"), f"{sustainability_score}/100"],
        ]
        ...
        if 'principles_checked' in scan_data and scan_data['principles_checked'] and isinstance(scan_data['principles_checked'], list):
            ...
            code_stats_data.append([_("report.gdpr_principles_checked", "GDPR Principles Checked"), f"{len(principles)}"])
        ...
        if 'principles_checked' in scan_data and scan_data['principles_checked'] and isinstance(scan_data['principles_checked'], list):
            ...
            principles_data = []
            principles_data.append([_("report.gdpr_principle", "GDPR Principle"), _("report.checked", "Checked")])
            
            # Get appropriate display names based on language
            for principle in principles:
                ...
                if current_lang == 'nl' and "rechtmatigheid" in all_principles:
        ...
        if language_breakdown:
            ...
            language_data = [[_("report.language", "Language"), _("report.files", "Files"), _("report.size_mb", "Size (MB)")]]
        ...
        if unused_imports:
            ...
            unused_data = [[_("report.file", "File"), _("report.import", "Import")]]
        ...
        if large_files:
            ...
            large_data = [[_("report.file", "File"), _("report.size_mb", "Size (MB)"), _("report.category", "Category")]]
